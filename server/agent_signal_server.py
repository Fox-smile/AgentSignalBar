...
================================
轻量级 HTTP 服务器，将 macOS/Linux 电脑上的 Agent 状态暴露给 Android 手机。
...
端点：
    GET /api/status  — 返回 /tmp/agent-signal/status.json 的内容
    GET /health      — 健康检查
    GET /            — 简单的状态概览页面（可在手机浏览器预览）
...
def get_display_name(aggregate: str | None) -> str:
    """将 aggregate 信号映射为中文显示名称。"""
    if aggregate is None:
        return "未知"
    mapping = {
        "idle": "空闲", "ready": "空闲",
        "thinking": "思考中", "working": "工作中", "tool_done": "步骤完成",
        "subagent_start": "子 Agent 运行中", "subagent_stop": "子 Agent 完成",
        "active": "工作中",
        "done": "已完成", "completed": "已完成",
        "attention": "需要查看", "notification": "通知",
        "permission": "等待授权", "permission_request": "等待授权",
        "blocked": "阻塞/失败", "failure": "失败", "error": "错误",
        "exception": "异常", "max_tokens": "上下文限制",
        "stale": "状态不可信",
        "off": "已关闭", "pause": "已暂停", "paused": "已暂停",
    }
    return mapping.get(aggregate, aggregate)


def get_css_class(aggregate: str | None) -> str:
    """根据 aggregate 返回 CSS 类名。"""
    if aggregate is None:
        return "unknown"
    ready = {"idle", "ready", "done", "completed", "session_start", "session_end", "turn_end"}
    active = {"thinking", "working", "tool_done", "subagent_start", "subagent_stop", "active"}
    blocked = {"blocked", "failure", "error", "exception", "max_tokens", "stale",
               "permission", "permission_request", "attention", "notification", "needs_review"}
    paused = {"off", "pause", "paused"}
    if aggregate in ready:
        return "ready"
    if aggregate in active:
        return "active"
    if aggregate in blocked:
        return "blocked"
    if aggregate in paused:
        return "paused"
    return "unknown"
    ...
    def do_GET(self):
        ...
        if path in ("/api/status", "/status.json"):
            ...
            if doc is None:
                ...
                self._send_json({
                    ...
                    "sessions": {},
                    "events": [],
                    "_note": "status.json 文件不存在，返回默认空闲状态"
                })
            else:
                self._send_json(doc)
        ...
        # ── /health ──
            ...
            self._send_json({
                ...
                "state_file_exists": doc is not None,
        ...
        # ── / (首页) ──
            ...
            for sid, rec in sessions.items():
                ...
                sessions_html += (
                    f'<div class="session">'
                    f'<strong>{agent}</strong>: {signal} '
                    f'<span style="color:#aaa">({sid[:12]}...)</span>'
                    f'</div>'
                )
